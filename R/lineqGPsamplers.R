...
tmvrnorm.RSM <- function(object, nsim, control = NULL, ...) {
  ...
  invSigmamu_star <-  invSigma %*% tmvPar$mu

  # generating the samples
  xi <- matrix(0, nrow = length(tmvPar$mu), ncol = nsim)
  for (i in seq(nsim)) {
    condition <- FALSE
    while (condition == FALSE) {
      xi_temp <- mvrnorm(n = 1, tmvPar$mu, Sigma = tmvPar$Sigma)
      while(!all(xi_temp >= tmvPar$lb & xi_temp <= tmvPar$ub)) {
        xi_temp <- mvrnorm(n = 1, tmvPar$mu, Sigma = tmvPar$Sigma)
      }
      t <- exp( t(tmvPar$mu - xi_temp) %*% invSigmamu_star)
      u <- runif(1)
...
tmvrnorm.Gibbs <- function(object, nsim,
  ...
  xi <- sample_from_constraints(constr$linear_part, constr$offset, tmvPar$mu,
                                tmvPar$Sigma, initial_point = tmvPar$mu,
...
tmvrnorm.HMC <- function(object, nsim, control = list(burn.in = 1e2), ...) {
  ...
  for (i in seq(length(control$constrType))) {
    ...
    M <- bdiag(M, lsys$M)
    g <- c(g, lsys$g)
    init_vec <- c(init_vec, pmin(pmax(tmvPar$mu[idxInit:idxEnd],
